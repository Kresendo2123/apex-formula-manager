...
class AdminApp(tk.Tk):
    ...
    def build_ui(self):
        ...
        self.tab_pit_details = ttk.Frame(self.notebook) # YENİ SEKME
        ...
        self.notebook.add(self.tab_pit_details, text="⏱️ Pit Stop Detayları")
        ...
        # Geliştirmeler, Sonuç ve Pit Detayları
        ...
        self.tree_pits = self.create_tree(self.tab_pit_details, "Yarış Stratejisi ve Pit Stop Analizi")
    ...
    def update_ui(self):
        ...
        for d_id, drv in u.drivers.items():
            ...
            drv_rows.append({"Pilot": drv.name, "Takım": u.teams[drv.team_id].name, "Puan": pts, "Sezon Başı Beklenti": u.exp_drv.get(d_id, "-")})
        ...
        for t_id, tm in u.teams.items():
            ...
            team_rows.append({"Takım": tm.name, "Puan": pts, "Beklenen Sıra": u.exp_team.get(t_id, "-")})
        ...
        stat_rows = []
        for d_id, drv in u.drivers.items():
            stat_rows.append({"Pilot": drv.name, "Takım": u.teams[drv.team_id].name, "Pace": drv.pace, "Consistency": drv.consistency, "Att/Def": drv.attack_defense, "Tire Mgt": drv.tire_management, "Potansiyel": drv.potential, "Kazanılan Perkler": ", ".join(drv.perks) if drv.perks else "Yok"})
        self.update_tree(self.tree_stats, pd.DataFrame(stat_rows))
        
        fac_rows = []
        for t_id, tm in u.teams.items():
            fac_rows.append({"Takım": tm.name, "Rüzgar Tüneli Lvl": tm.facilities.get("wind_tunnel", 1), "Simülatör Lvl": tm.facilities.get("simulator", 1), "Fabrika Lvl": tm.facilities.get("factory", 1), "Mevcut İnşaat": f"{tm.active_facility_upgrade} (Kalan: {tm.facility_upgrade_remaining_races} Yarış)" if tm.active_facility_upgrade else "Boşta"})
        self.update_tree(self.tree_fac, pd.DataFrame(fac_rows))
        
        # Son Geliştirmeler, Yarış ve Pit Detayları
        ...
        self.update_tree(self.tree_pits, pd.DataFrame(u.latest_pit_details))
